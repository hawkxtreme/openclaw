...
describe("scripts/docker/setup.sh", () => {
  ...
  it("applies OPENCLAW_DOCKER_CONFIG_BATCH_FILE before starting the gateway", async () => {
    const activeSandbox = requireSandbox(sandbox);
    const extraBatchJson =
      '[{"path":"models.providers.ollama.baseUrl","value":"http://host.docker.internal:11434"},{"path":"agents.defaults.model.primary","value":"ollama/qwen3.5:9b"}]';
    const batchFilePath = join(activeSandbox.rootDir, "docker-config-batch.json");

    await writeFile(batchFilePath, `${extraBatchJson}\n`);
    await resetDockerLog(activeSandbox);
    const result = runDockerSetup(activeSandbox, {
      OPENCLAW_DOCKER_CONFIG_BATCH_FILE: batchFilePath,
    });
